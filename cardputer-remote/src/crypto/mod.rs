...
impl CryptoContext {
    ...
    pub fn set_peer_public_key(&mut self, public_key_hex: &str) -> Result<(), CryptoError> {
        ...
        self.peer_public_key = Some(public_key.unwrap());
        Ok(())
    }
    ...
    pub fn derive_session_keys(
        ...
        if peer_public.is_none().into() {
            return Err(CryptoError::InvalidPublicKey);
        }
        let peer_public = peer_public.unwrap();
        ...
        let session_keys = SessionKeys {
            client_to_server_key: okm[0..16].try_into().unwrap(),
            server_to_client_key: okm[16..32].try_into().unwrap(),
            hmac_key: okm[32..64].try_into().unwrap(),
        };
        ...
        self.outgoing_cipher = Some(Aes128Gcm::new_from_slice(outgoing_key).unwrap());
        self.incoming_cipher = Some(Aes128Gcm::new_from_slice(incoming_key).unwrap());
        self.session_keys = Some(session_keys);

        // Reset nonce counters
        self.outgoing_nonce_counter = 0;
        self.last_incoming_nonce = 0;
